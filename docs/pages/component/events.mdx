...
<details>
  ...
  | `AD_CAN_PLAY`                | Android, iOS | Fires when the ad is ready to play without buffering, either at the beginning of the ad or after buffering completes.                                                                                       |
  ...
  | `ADS_MANAGER_LOADED`         | iOS          | Fires when ads are successfully loaded from the ad servers by the loader. The event `data` will be populated with `adCuePoints`.                                                                            |
